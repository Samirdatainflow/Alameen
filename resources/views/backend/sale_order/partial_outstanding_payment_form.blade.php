{{ Form::open(array('id'=>'CreatePartialOutstanding')) }}
    <div class="row">
        @php
                ...
                if(!empty($ClientData)) {
                    ...
                    if(!empty($SaleOrder)) {
                        if(!empty($SaleOrder[0]['client_id'])) {
                          if($SaleOrder[0]['client_id'] == $client['client_id']) $sel='selected="selected"';
                        }
                    }
                @endphp
                <option value="{{$client['client_id']}}" {{$sel}}>{{ $client['customer_name']}}</option>
                @php
                    }
                }
                @endphp
        ...
        <div class="col-md-2">
            <button type="button" id="get_customer_partial_details" class="btn-shadow btn btn-info valid" value="Submit" aria-invalid="false"> Load </button>
        </div>
        <div class="col-md-3 hide-amount-field" style="display:none;position: relative;top: -24px;">
            <lable>Total Invoice Due Amount</lable>
            <input type="number" class="form-control" id="partial_total_invoice_amount" name="partial_total_invoice_amount" readonly>
        </div>
        <div class="col-md-3 hide-amount-field" style="display:none;position: relative;top: -24px;">
            <lable>Enter Payment Amount</lable>
            <input type="number" class="form-control" id="partial_balance_amount" name="partial_balance_amount" placeholder="Enter Amount">
        </div>
    </div>
    <div class="row">
        <p>&nbsp;</p>
